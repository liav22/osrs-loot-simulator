...
describe('both sources are off the mechanics watchlist', () => {
  ...
  /**
   * This assertion has now been through both halves of a full cycle, and the
   * history is worth keeping rather than flattening.
   *
   * It originally asserted `status === 'verified'` for both. That stopped
   * holding when `drops_covered` shipped — not a regression here, but the new
   * check correctly reporting that both sources lost transcluded sub-tables
   * the parser never saw (Abyssal Sire's Seeds and Talismans, Corporeal
   * Beast's Sigils), so neither document was complete whatever else it got
   * right. It was narrowed then to "every structural check passes and coverage
   * is the only thing outstanding."
   *
   * `expand-transclusions.ts` closed that gap, so it is back to asserting
   * every deterministic check passes — coverage included.
   */
  it('pass every deterministic check, coverage included', async () => {
